...
//Interrupt handler that takes higher priority during its execution.
...
u8 MyReceiveFunction( void ){
	...
	while (uxQueueMessagesWaiting(xQueue_for_receive) == 0){
	}

	xQueueReceive(xQueue_for_receive, &recv, 0);
