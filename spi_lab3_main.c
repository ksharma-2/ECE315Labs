	...
	while(1){
		...
		if(flag==1){ 						// flag is set to 1 when the user enters the end sequence on SPI0-SPI1 mode
			...
			for(int i=0;i<loop_counter;i++){
				...
				xQueueSendToBack(xQueue_FIFO1, &dummy, 0);
	...
	while(1){
		...
		if(spi_master_loopback_en==1 && current_command_execution_flag==2) 				//just send the characters back to task 1. No SPI access! This is because loopback is currently enabled.
			...
			if(bytecount == TRANSFER_SIZE_IN_BYTES){	//byte is read from the FIFO1 and then it is transferred to the SPI connection
				...
				send_buffer[1] = task2_receive_from_FIFO1; //copying data.
				SpiMasterWrite(send_buffer, TRANSFER_SIZE_IN_BYTES);
				taskYIELD();

				send_buffer[1] = SpiMasterRead(TRANSFER_SIZE_IN_BYTES);
				xQueueSendToBack(xQueue_FIFO2, send_buffer, 0);
				bytecount = 0;
...
static void TaskSpi0Peripheral( void *pvParameters ){
	...
	while(1){
		...
		if(spi_master_loopback_en==0 && current_command_execution_flag==2){
			...
			if(temp_store == CHAR_CARRIAGE_RETURN && end_sequence_flag == 2) {
				...
				for(int i = 0; i < sizeof(buffer); i++){
					RxBuffer_Slave[0] = buffer[i];
					SpiSlaveWrite(RxBuffer_Slave, TRANSFER_SIZE_IN_BYTES);
				}
				end_sequence_flag += 1;
			} else if (temp_store == CHAR_PERCENT && end_sequence_flag == 1){
				end_sequence_flag += 1;
		...
		RxBuffer_Slave[1] = temp_store;
		SpiSlaveWrite(RxBuffer_Slave, TRANSFER_SIZE_IN_BYTES);

		num_received += 1; //Adding one to the number of characters read.
...
void executeSpiMasterCommand(void){
	...
	//If there is space in the UART transmitter, write the byte to the UART
		...
		xQueueReceive(xQueue_FIFO2, &task1_receive_from_FIFO2_spi_data, 0);
		while (XUartPs_IsTransmitFull(XPAR_XUARTPS_0_BASEADDR) == TRUE){};
